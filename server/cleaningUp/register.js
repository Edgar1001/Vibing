...
//name checks
	...
	pool.query('SELECT * FROM users WHERE username = $1 OR email = $2',
		...
		(err, result) => {
			if (err) {
				console.error('Error executing SELECT query:', err); // Log the actual error for debugging
				return res.status(500).send("Database query failed."); // Send a generic error response
			}
	
			if (result && result.rowCount > 0) {
				// If a user with the same username or email exists, return a message
				return res.status(400).send({ message: `Username / email already exists` });
			} else {
				// If no duplicate found, proceed to hash the password
				bcrypt.hash(password, saltRounds, (err, hash) => {
					if (err) {
						console.error('Error hashing password:', err); // Log error
						return res.status(500).send("Error hashing password."); // Send an error response
					}
					...
					(err, result) => {
						if (err) {
							...
							return res.status(500).send("Error inserting user into database."); // Send an error response
						} else {
							return res.send({ message: `Registration email sent to '${email}'`, result });
						}
					});
				});
			}
...
const sendVerifyCode = () => {
    const theMail = {
        ...
        html: `
            ...
            <a href="${config.BASE_URL}/emailverify/${verifycode}">HERE</a><br />
            <p>Thanks.</p>`
    };
    ...
    transporter.sendMail(theMail, (err, info) => {
        if (err) {
    ...
    (err, result) => {
        ...
        if (result && result.rowCount > 0) {
            ...
            bcrypt.hash(password, saltRounds, (err, hash) => {
                ...
                sendVerifyCode(); // Send verification email
                ...
                pool.query("INSERT INTO users (username, firstname, lastname, email, password, verifycode) VALUES ($1, $2, $3, $4, $5, $6)",
                    ...
                    (err, result) => {
                        ...
                        return res.send({ message: `Registration email sent to '${email}'` });
                    });
            });
        }
    });
