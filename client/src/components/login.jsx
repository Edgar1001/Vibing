...
function useLogin() {
  ...
  const handlePassword = (event) => {
    ...
    await axiosStuff.login(login2).then((response) => {
      ...
      if (response.result) {
        const userData = response.result.rows[0]; // Access the first user record
        setLoginsStatus(userData.username);
        
        // Skipping the email verification check
        // Uncomment this block to keep original checks
        /*
        ...
        */
        ...
        // Allow all logged in users to go to the browse page for testing
        console.log("User logged in, redirecting to /browse");
